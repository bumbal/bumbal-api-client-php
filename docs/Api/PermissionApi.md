...
$api_instance = new BumbalClient\Api\PermissionApi();
...
try {
    $result = $api_instance->retrieveListPermission($arguments);
    print_r($result);
} catch (Exception $e) {
...
try {
    $result = $api_instance->retrievePermissionId($permission_id);
    print_r($result);
} catch (Exception $e) {
