...
$apiInstance = new BumbalClient\Api\ActivityTypeApi(
...
try {
    $result = $apiInstance->retrieveActivityType($activity_type_id);
    print_r($result);
} catch (Exception $e) {
...
try {
    $result = $apiInstance->retrieveListActivityType($arguments);
    print_r($result);
} catch (Exception $e) {
